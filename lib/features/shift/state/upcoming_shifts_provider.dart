...
import '../services/add_shif_services.dart';
...
class UpcomingShiftsNotifier extends StateNotifier<AsyncValue<List<Shift>>> {
  ...
  void streamUserUpcomingShifts({required String profileEmail}) {
    ...
    _shiftSubscription = ShiftServices.streamUpcomingShiftsByEmail(email: profileEmail).listen(
          (shifts) {
        state = AsyncValue.data(shifts);
      },
      onError: (error) {
        state = AsyncValue.error('Failed to fetch user shifts: $error', StackTrace.current);
      },
    );
  }
